...
class TestFileLoading(TestCase):
    def __init__(self, *args, **kwargs):
        ...
        """ Initialize unchanged class attributes for testing
            ( act similar to global variables )
        Notes:
        ...
        self.unpickled_list = pd.read_pickle(kinematic_data_path)
...
class TestGetMethods(TestCase):
    ...
    def test_get_kinematic_block(self):
        """
        Tests:
            function returned desired block
        """
        # assumes rat,date,session exist
        rat = 'RM16'
        kdate = '0190920'
        session = 'S3'
        block_df = CU.get_kinematic_block(self.kinematic_df, rat, kdate, session)

        # index should correspond to desired block
        expected_df_index = ('RM16', '0190920', 'S3', 0)
        self.assertEqual(expected_df_index, block_df.index.values[0])
