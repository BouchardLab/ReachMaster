...
def single_plot(X,Y,Z,x_rewz_s,y_rewz_s,z_rewz_s,savepath=False):
    ...
    ZN=np.ravel(Z)
    ax.plot_surface(XN,YN,ZN.reshape(ZN.shape[0],1),rstride=2,cstride=2,color='b',alpha=0.4,linewidth=0.0001, edgecolors='g')
    ax.set_xlabel('x(mm)')
    ax.set_ylabel('y(mm)')
    ax.set_zlabel('z(mm)')
    plt.legend()
...
def forward_xform_coords(x,y,z):
    ...
    cos_bot= (2*Az_est*Lz)
    r3 = np.sqrt(Az_est**2+(Ly-Lz)**2-(2*Az_est*(Ly-Lz)*np.cos(np.pi-np.arccos((Az_est**2+Lz**2-sd)
                                                                                  /(2*Az_est*Lz)))))
